;((global, factory) => {
  ...
  function isFixed(a) {
    return (
      a !== window.document.body &&
      (!("fixed" !== getStyleComputedProperty(a, "position")) || (getParentNode(a) ? isFixed(getParentNode(a)) : a))
    )
  }
  ...
  function getOuterSizes(a) {
    ...
    var d = window.getComputedStyle(a),
      e = Number.parseFloat(d.marginTop) + Number.parseFloat(d.marginBottom),
      f = Number.parseFloat(d.marginLeft) + Number.parseFloat(d.marginRight),
  ...
  function isNumeric(a) {
    return "" !== a && !isNaN(Number.parseFloat(a)) && isFinite(a)
  }

  function isTransformed(a) {
    return (
      a !== window.document.body &&
      ("none" !== getStyleComputedProperty(a, "transform") || (getParentNode(a) ? isTransformed(getParentNode(a)) : a))
    )
  }
  ...
  var isNative = (a) => nativeHints.some((b) => -1 < (a || "").toString().indexOf(b))
  ...
  function arrow(a, b) {
    ...
    return (
      (o = Math.max(Math.min(f[i] - m, o), 0)), (d[j] = o), (d[k] = ""), (a.offsets.arrow = d), (a.arrowElement = c), a
    )
  }

  function getOppositePlacement(a) {
    var b = { left: "right", right: "left", bottom: "top", top: "bottom" }
    return a.replace(/left|right|bottom|top/g, (c) => b[c])
  }
    ...
    var g = d.priority,
      ...
      i = {
        ...
        right: function right() {
          var j = h.left
          return (
            h.right > f.right && !shouldOverflowBoundary(c, d, "right") && (j = Math.min(h.left, f.right - h.width)),
            { left: j }
          )
        },
        top: function top() {
  ...
  function hide(a) {
    ...
    var b = a.offsets.reference,
      c = a.instance.modifiers.filter((d) => "preventOverflow" === d.name)[0].boundaries
  ...
  var Popper = (() => {
    function Popper(a, b) {
      ...
      return (
        ...
        (this.modifiers = Object.keys(DEFAULTS.modifiers).map((d) =>
          Object.assign({ name: d }, DEFAULTS.modifiers[d]),
        )),
        (this.modifiers = this.modifiers.map((d) => {
        ...
        c.modifiers &&
          ...
          Object.keys(c.modifiers).forEach((d) => {
            if (void 0 === DEFAULTS.modifiers[d]) {
              var e = c.modifiers[d]
              ;(e.name = d), this.modifiers.push(e)
            }
          })),
        (this.modifiers = this.modifiers.sort(sortModifiers)),
        this.modifiers.forEach((d) => {
          d.enabled && isFunction(d.onLoad) && d.onLoad(this.reference, this.popper, this.options, d)
        }),
