...
;((factory) => {
	  ...
	  _untrackClassesElement: function (event) {
		$.each(this.classesElementLookup, (key, value) => {
		  if ($.inArray(event.target, value) !== -1) {
			this.classesElementLookup[key] = $(value.not(event.target).get())
		  }
		})
	  },
	  ...
	  var stepHooks =
		...
		stringParsers = [
		  ...
		  },
		  {
			...
			parse: (execResult) => [execResult[1] * 2.55, execResult[2] * 2.55, execResult[3] * 2.55, execResult[4]],
		  },
		  {
		  ...
		  },
		  {
			...
			space: "hsla",
			parse: (execResult) => [execResult[1], execResult[2] / 100, execResult[3] / 100, execResult[4]],
		  },
		],
		// JQuery.Color( )
	...
	;(() => {
		...
		transfer: function (options, done) {
		  var element = $(this),
			...
			transfer = $("<div class='ui-effects-transfer'></div>")
			  ...
			  .animate(animation, options.duration, options.easing, () => {
				transfer.remove()
				if ($.isFunction(done)) {
				  done()
				}
	  ...
	  function parseClip(str, element) {
		...
		return {
		  top: Number.parseFloat(values[1]) || 0,
		  right: values[2] === "auto" ? outerWidth : Number.parseFloat(values[2]),
		  bottom: values[3] === "auto" ? outerHeight : Number.parseFloat(values[3]),
		  left: Number.parseFloat(values[4]) || 0,
		}
	  }
	...
	;(() => {
	  ...
	  $.extend(baseEasings, {
		...
		Elastic: (p) =>
		  p === 0 || p === 1 ? p : -Math.pow(2, 8 * (p - 1)) * Math.sin((((p - 1) * 80 - 7.5) * Math.PI) / 15),
		Back: (p) => p * p * (3 * p - 2),
		Bounce: (p) => {
		  var pow2,
			bounce = 4
	...
	var effectsEffectSize = $.effects.define("size", function (options, done) {
	  ...
	  if (scale === "content" || scale === "both") {
		...
		element.find("*[width]").each(function () {
		  ...
		  // Animate children
		  child.css(childFrom)
		  child.animate(childTo, options.duration, options.easing, () => {
			// Restore children
			if (restore) {
	...
	var effectsEffectScale = $.effects.define("scale", function (options, done) {
	  ...
	  var el = $(this),
		mode = options.mode,
		percent =
		  Number.parseInt(options.percent, 10) ||
		  (Number.parseInt(options.percent, 10) === 0 ? 0 : mode !== "effect" ? 0 : 100),
		newOptions = $.extend(
		  true,
		  {
	...
	var effectsEffectPuff = $.effects.define("puff", "hide", function (options, done) {
	  var newOptions = $.extend(true, {}, options, {
		fade: true,
		percent: Number.parseInt(options.percent, 10) || 150,
	  })
	...
	$.extend($.expr[":"], {
	  focusable: (element) => $.ui.focusable(element, $.attr(element, "tabindex") != null),
	})
	...
	var escapeSelector = ($.ui.escapeSelector = (() => {
	  ...
	  return (selector) => selector.replace(selectorEscape, "\\$1")
	})())
	...
	var safeActiveElement = ($.ui.safeActiveElement = (document) => {
	  var activeElement
	...
	$.widget("ui.autocomplete", {
	  ...
	  _renderItem: (ul, item) => $("<li>").append($("<div>").text(item.label)).appendTo(ul),
	...
	$.widget("ui.autocomplete", $.ui.autocomplete, {
	  options: {
		messages: {
		  ...
		  results: (amount) =>
			amount + (amount > 1 ? " results are" : " result is") + " available, use up and down arrow keys to navigate.",
		},
	  },
	...
	var widgetsControlgroup = $.widget("ui.controlgroup", {
	  ...
	  _initWidgets: function () {
		...
		$.each(this.options.items, (widget, selector) => {
		  var labels
		  var options = {}
	  ...
	  refresh: function () {
		...
		if (children.length) {
		  ...
		  $.each(["first", "last"], (index, value) => {
			var instance = children[value]().data("ui-controlgroup-data")
  
			if (instance && this["_" + instance.widgetName + "Options"]) {
			  var options = this["_" + instance.widgetName + "Options"](children.length === 1 ? "only" : value)
			  options.classes = this._resolveClassesValues(options.classes, instance)
			  instance.element[instance.widgetName](options)
			} else {
			  this._updateCornerClass(children[value](), value)
			}
		  })
	...
	$.extend(Datepicker.prototype, {
	  ...
	  _enableDatepicker: function (target) {
		...
		this._disabledInputs = $.map(this._disabledInputs, (value) => (value === target ? null : value)) // delete entry
	  },
	  ...
	  _shouldFocusInput: (inst) =>
		inst.input && inst.input.is(":visible") && !inst.input.is(":disabled") && !inst.input.is(":focus"),
	...
	var widgetsMouse = $.widget("ui.mouse", {
	  ...
	  _mouseInit: function () {
		this.element
		  .on("mousedown." + this.widgetName, (event) => this._mouseDown(event))
	...
	var safeBlur = ($.ui.safeBlur = (element) => {
	  // Support: IE9 - 10 only
	...
	$.widget("ui.draggable", $.ui.mouse, {
	  ...
	  _mouseStop: function (event) {
		...
		if (
		  ...
		  $(this.helper).animate(this.originalPosition, Number.parseInt(this.options.revertDuration, 10), () => {
	  ...
	  _setContainment: function () {
		...
		this.containment = [
		  (Number.parseInt(c.css("borderLeftWidth"), 10) || 0) + (Number.parseInt(c.css("paddingLeft"), 10) || 0),
		  (Number.parseInt(c.css("borderTopWidth"), 10) || 0) + (Number.parseInt(c.css("paddingTop"), 10) || 0),
	  ...
	  start: function (event, ui, instance) {
		var min,
		  ...
		  group = $.makeArray($(o.stack)).sort(
			(a, b) => (Number.parseInt($(a).css("zIndex"), 10) || 0) - (Number.parseInt($(b).css("zIndex"), 10) || 0),
		  )
	...
	$.widget("ui.resizable", $.ui.mouse, {
	  ...
	  _num: (value) => Number.parseFloat(value) || 0,
  
	  _isNumber: (value) => !isNaN(Number.parseFloat(value)),
	  ...
	  _mouseStop: function (event) {
		...
		if (this._helper) {
		  ...
		  left = Number.parseFloat(this.element.css("left")) + (this.position.left - this.originalPosition.left) || null
		  top = Number.parseFloat(this.element.css("top")) + (this.position.top - this.originalPosition.top) || null
	  ...
	  _getPaddingPlusBorderDimensions: (element) => {
		...
		for (; i < 4; i++) {
		  widths[i] = Number.parseFloat(borders[i]) || 0
		  widths[i] += Number.parseFloat(paddings[i]) || 0
		}
	...
	$.ui.plugin.add("resizable", "animate", {
	  stop: function (event) {
		var that = $(this).resizable("instance"),
		  ...
		  left = Number.parseFloat(that.element.css("left")) + (that.position.left - that.originalPosition.left) || null,
		  top = Number.parseFloat(that.element.css("top")) + (that.position.top - that.originalPosition.top) || null
		...
		that.element.animate($.extend(style, top && left ? { top: top, left: left } : {}), {
		  ...
		  step: () => {
			var data = {
			  width: Number.parseFloat(that.element.css("width")),
			  height: Number.parseFloat(that.element.css("height")),
			  top: Number.parseFloat(that.element.css("top")),
			  left: Number.parseFloat(that.element.css("left")),
			}
	  ...
	  start: function () {
		...
		$(o.alsoResize).each(function () {
		  ...
		  el.data("ui-resizable-alsoresize", {
			width: Number.parseFloat(el.width()),
			height: Number.parseFloat(el.height()),
			left: Number.parseFloat(el.css("left")),
			top: Number.parseFloat(el.css("top")),
		  })
		})
	  },
	...
	*/
	  ...
	  $.ui.formResetMixin,
	  {
		...
		_parseOptions: function (options) {
		  ...
		  options.each((index, item) => {
			data.push(this._parseOption($(item), index))
		  })
		  this.items = data
		},
	...
	var widgetsSortable = $.widget("ui.sortable", $.ui.mouse, {
	  ...
	  _isFloating: (item) => /left|right/.test(item.css("float")) || /inline|table-cell/.test(item.css("display")),
	...
	$.widget("ui.tabs", {
	  ...
	  _create: function () {
		...
		if ($.isArray(options.disabled)) {
		  options.disabled = $.unique(
			options.disabled.concat($.map(this.tabs.filter(".ui-state-disabled"), (li) => this.tabs.index(li))),
		  ).sort()
		}
	  ...
	  _sanitizeSelector: (hash) => (hash ? hash.replace(/[!"$%&'()*+,./:;<=>?@[\]^`{|}~]/g, "\\$&") : ""),
	  ...
	  _createPanel: (id) => $("<div>").attr("id", id).data("ui-tabs-destroy", true),
	  ...
	  load: function (index, event) {
		...
		var tab = this.tabs.eq(index),
		  ...
		  complete = (jqXHR, status) => {
			...
			this._removeClass(tab, "ui-tabs-loading")
	  ...
	  _ajaxSettings: function (anchor, event, eventData) {
		return {
		  ...
		  beforeSend: (jqXHR, settings) =>
			this._trigger("beforeLoad", event, $.extend({ jqXHR: jqXHR, ajaxSettings: settings }, eventData)),
		}
	  },
